...
Concrete classes, interfaces, and enums for the launch implementation. Derived from [tech notes](04-tech-notes.md), [release scope](00-release-scope.md), [ADR 014�016](../decisions/), and all locked system docs.

> **Status:** design draft � structure locked for required slice; **game phase flow locked** ([ADR 017](../decisions/017-game-phase-controller.md), [game phase](02-systems/game-phase.md)); other naming open for bikeshedding before first `.cs` files commit.
...
{
    string locationId;            // "s1", "sd01", � � location-neutral ([ADR 040](../../decisions/040-floor-exit-topology-graph.md))
    string floorId;               // "B1F"
    int gridWidth;
    int gridHeight;
...
class HubController : MonoBehaviour
{
    ...
    // optional � see � optional side dungeons sketch
...
// GridDungeon.Editor � floor level painter ? ExplorationFloor.asset (ADR 002, #107)
...
## Side dungeons (sketch) � side dungeons (sketch)
...
`ContentDatabase.GetFloor` may resolve by composite key or `(MapKind, locationId, floorId)` � implementation detail for optional side dungeons.
