...
| **Portrait (right)** | Large character illustration | **Deferred** — 3D character preview lives on a **separate surface** (not `CharacterDetail` modal) |
...
| Portrait on status pane | **Optional later** — not required for equipment pane parity |
