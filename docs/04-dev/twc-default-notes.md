...
| **`Walls`** build prototype | Editor bootstrap only — cloned for each **`Level_sN`** at runtime | Dual grid + walls tile preset; `_WallsPrototype` blueprint is never painted |
...
| ----- | ------ | ------------ |
| **`Level_sN`** | `FloorArtTwcElevationTranslator` — walkable terraces, layout walls, chest pads | Dual-grid **walls preset** on every step — unified mesh per elevation layer |

Flat terrain: clear or zero `cellElevationSteps` on the floor asset — runtime still provisions **`Level_s0`** (no separate flat-mode path).

Center-pivot TWC tiles: `layerYOffset = -twcCellSize/2` (2.5 u when sub-grid cellSize is 5). Fixed offsets avoid door/chest raycast misalignment from `placeOnTop`.

**Runtime:** `FloorArtTwcTileLayerYOffset` resolves pivot from preset mesh bounds. Elevation terrace stack uses blueprint `defaultLayerHeight = step × twcCellSize`; `layerYOffset` is pivot-only.
...
Chest pads (`HasChest`, non-walkable) are painted by `FloorArtTwcElevationTranslator` on the terrace surface step.

Re-run **Setup B1F** to strip removed legacy blueprint/build layers from older configs (`Walkable`, `WalkableFloor`, `SolidMass`, `Floor`, …).
...
**Manual alternative:** edit the **Walls** build layer on `GridDungeonTwcConfiguration.asset` directly, or leave **Tileset Profile** empty to use whatever is saved on the configuration.
...
Applied at runtime to **Walls** prototype and each **`Level_sN`** build layer (`scaleTileToCellSize` stays on). Does **not** change logic grid or `cellSize`.
...
| Input mask           | Painted cells per `Level_sN` (terraces + walls)                                                                  | Same                                                    |
| Corridors            | Terrace columns fill walkable cells — no separate grass-under-walls layer                                      | Same                                                    |
...
- **FPV mesh (rendering only):** one unified **`Level_sN`** blueprint + build per calculated elevation step. Walls preset / dual-grid on every level layer.
- **Walkable terrace:** foundation blocks on layers `0..surfaceStep` (authored `cellElevationSteps`). All-zero elevation → single **`Level_s0`** terrace column.
...
- **TWC vertical stack:** blueprint **`defaultLayerHeight = layerStep × twcCellSize`**. Build **`layerYOffset = -twcCellSize/2`** is mesh pivot align only. **`placeOnTop`** disabled (door/chest misalignment).
...
Alignment sample (when enabled) logs `Level_sN yOffset=… height=…` for the active step range.
...
| Walls fill corridor / on walkable cells        | Wrong blueprint cells painted                                                | Verify `FloorArtTwcElevationTranslator` buckets per `Level_sN` |
...
| Missing ground mesh                          | Terrace paint empty or walls preset missing                                   | Re-run **Setup B1F**; confirm **Walls** prototype exists on config                                  |
...
| Hole in floor at chest                         | Chest cells are not walkable                                                 | `FloorArtTwcElevationTranslator` paints chest pads on surface step                                    |
...
| `Layer not found: Walls`                       | Stale or hand-edited config                                                  | Re-run **Setup B1F** menu                                                                             |
...
| `FloorArtTwcWalkableMaskBuilder` | `ExplorationFloor` → `HashSet<Vector2>` (`BuildWalkableCells`, `ExpandToTwcBlueprint`) — tests / helpers |
...
**Committed `GridDungeonTwcConfiguration.asset`:** slim bootstrap template (`_WallsPrototype` blueprint + disabled **Walls** build prototype — no pre-authored `Level_sN`). Regenerate via **Setup B1F** after TWC import; Play Mode clones at runtime and provisions elevation layers in code.
