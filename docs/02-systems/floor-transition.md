...
| Element | Behavior |
|---------|----------|
| `DungeonSceneHost` / floor art | Screen fade covers the leave-floor slice; **`DungeonSceneHost` stays active** during unload/load so `FloorArtPresenter` coroutines are not stopped. Hub destination hides `DungeonSceneHost` after a successful handoff. |
