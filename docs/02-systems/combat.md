...
**UI:** Command bar **Back button**; enabled when targeting or when LIFO is available. **Global input hint** (bottom-right `InputHintPresenter`): context copy from `TabbedPickerRailHints` — e.g. **Z Confirm · X Cancel · WASD Navigate · L Log · Esc Pause** during core planning; **X Close · L Close** while the log modal is open ([shared menu & picker UI § Global input hints](../04-dev/shared-menu-picker-ui.md#global-input-hints)). Per-panel footer hints removed. Roster **queued** styling clears when a command is popped.
...
**Locked UX (unchanged from MVP1 skeleton):** core command turns highlight the **party roster** slot, not the AGI strip; reactive beats and `CombatPresentationGate` pacing unchanged ([#35](https://github.com/miramocha/griddungeon-game/issues/35)).
...
Combat must show a **vertical strip** on the **right rail** (top → bottom = soonest → latest) listing **every combatant in the current round’s AGI queue** (flat queue — no front/back grouping):
