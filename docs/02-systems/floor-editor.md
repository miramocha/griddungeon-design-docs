...
5. **Elevation:** edit **step units**, **blocky terrain**, seed/min/max, then **Apply** (current seed), **Randomize** (new seed + apply), or **Clear**; map pane shows a **step heatmap** (blue low → amber high) with **step numbers** on each cell while this tab is active; **Save** writes `cellElevationSteps`, `elevationStepUnits`, and `useBlockyTerrain` on the floor asset.
