...
| **Select tab** | LMB on tab header | Optional; **`Q`/`E`** sufficient for MVP1 |
...
**Deferred (not MVP1):** gamepad tab cycle — **`L1`** / **`R1`** when [ADR 009](009-input-bindings-pc.md) gamepad support ships; reserve action names in the action map, no implementation required for picker MVP1.
...
Tab id → `SkillType` mapping lives in Core (single table). **No** separate `pickerTabId` on content in MVP1 unless a follow-up ticket needs finer UX than `SkillType`.
...
| **Field** | Party menu / pause **Use skill** (post-MVP1 slice) | Allocated skills with Field flag; ADR 034 gates (not combat / VN / cutscene) |
