...
EO IV **row promote** ([#377](https://github.com/miramocha/griddungeon-game/issues/377) / ADR 044) may land in parallel; this ADR locks Switch behavior and documents promote interaction.
...
| **AGI queue** | Mid-round Switch does **not** rebuild the AGI queue |

### 2. UX — pick-two on combat floater

1. Player highlights a **living core** and picks **Switch** on the command rail.
...
4. Command queues as **Switch** with the two **core indices**; planning advances to the next actor.
5. **Cancel:** `X` / Back clears pick mode without queuing (like target cancel).

**Summons / guests / aux:** Switch is **not** on summon or guest planning menus. **NPC scripted guests** never Switch.

### 3. Scope — core slots 0–5 only

- Only **core grid slots** `0..5` (`BattleFormation.MaxCoreSlots`).
- **No aux** slot swapping in combat Switch.
...
4. If the swap is a no-op (same index) or fails validation → turn still resolves with Synchro gain but **no formation change**.
5. **Dead switcher at AGI:** if the acting core is **dead** when their AGI slot arrives, the turn is **skipped** (existing dead-turn skip) — Switch does not resolve.
...
| **Switch** | Runs on the **queued core’s AGI turn** |

**Order:** Promote from a kill on enemy turn **N** completes before that round’s remaining AGI actions. A Switch queued in **planning** for core **A** resolves on **A**’s AGI slot — may be before or after a promote on the same round depending on AGI order; both apply to `BattleState` independently. **No AGI rebuild** after either.

If promote and Switch target the **same slot** in one round, **resolve order is AGI order** — whichever beat runs first wins that moment’s slot contents; the later beat applies to current state.
...
| Option | Why |
|--------|-----|
| Free mid-fight Switch (EO IV+) | Scope; turn cost is EO II default |
| Switch aux / guest slots | Core reposition only at launch |
| Rebuild AGI on Switch | Loses queued turn order; EO II keeps queue |
| Instant Switch on menu pick | Must match Guard / Attack resolve on AGI turn |

## Consequences

- [combat.md](../docs/02-systems/combat.md) — `Switch` in core command table
