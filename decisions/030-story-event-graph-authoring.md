...
[ADR 028](028-story-visual-novel-events.md) locks **MVP1** story delivery to a **linear step list** (`line`, `effect`) with **`choice` / branching deferred** but schema reserved (`gotoStep`, `branchId`). Runtime triggers stay in C# phase controllers ([#87](https://github.com/miramocha/griddungeon-game/issues/87)).
...
**Rule:** The graph is **authoring-only**. Shipping builds consume **compiled** definitions (ScriptableObject or baked YAML under `Assets/Content/StoryEvents/`). No graph interpreter in player code for MVP1+.
...
| **Branch** (post-MVP1) | implicit routing | **Condition** node: evaluate campaign flag / expression → pick edge |
...
|-----------|-------------|
| **MVP1 ([#87](https://github.com/miramocha/griddungeon-game/issues/87))** | Hand-authored linear definitions (YAML or SO); **no graph editor required** |
| **Post-MVP1 — schema** | Enable `choice` in runner + content; S1 scenes stay linear unless redesign needs a branch |
| **Post-MVP1 — editor** | Graph asset + compile to `StoryEventDefinition`; migrate S1 drafts from markdown → graph export |
...
- **Design:** [story-events.md](../docs/02-systems/story-events.md) — add § Graph authoring (link this ADR); keep MVP1 path linear.
