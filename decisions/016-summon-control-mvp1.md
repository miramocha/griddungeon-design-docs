...
- **`deploy_scout_drone`** may be **queued** while aux back is occupied (e.g. player expects current drone to die before the summoner’s AGI turn). On **resolve**, if the slot is still occupied: **fail**, **no MP spent**, player feedback ([class-skills](../docs/03-content/class-skills.md#locked-implementation-rules)).
