...
description: >-
  Validates Grid Dungeon S1 floor assets (walkability, funnel paths, exit links)
...
| Rule | Value |
|------|--------|
| Grid | **21×21** (`ExplorationGridMetrics.FloorGridCells`) |
...
2. **Validate row width** — grid must be **21×21** for default S1 floors.
...
`FloorEditorValidationTests` loads committed B1F/B2F/B3F assets. Domain tests (`ExplorationPathGraphTests`, `GatherInteractorTests`, `FoeSystemPatrolTests`, …) assert gameplay against committed floor assets via `FloorAssetFixtures` / `S1CampaignResolver`.
